#!/usr/bin/env python3
"""
Restore XCF v9 — v8 with Wiring layers preserved from v7.

Takes the v8 approach (append clean tile data onto v7 base) but SKIPS
layers whose names contain "Wiring" — those have newer edits in v7
that should be preserved. Only the large image layers (top, bottom, etc.)
get their tiles replaced from the clean file.
"""
...
def parse_layer_tiles(d, layer_off):
    """Parse layer and return tile info for level 0."""
    w = u32(d, layer_off)
    h = u32(d, layer_off + 4)
...
# Layers to keep from v7 (newer edits, not damaged)
KEEP_FROM_V7 = ['Wiring']


def should_keep_from_v7(name):
    """Return True if this layer should keep v7 tile data (not replaced from clean)."""
    for keyword in KEEP_FROM_V7:
...
def main():
    if len(sys.argv) < 3:
        print("Usage: restore_xcf_v9.py <v7-restored.xcf> <clean.xcf> [output.xcf]")
        sys.exit(1)

    v7_path = sys.argv[1]
    clean_path = sys.argv[2]
    output_path = sys.argv[3] if len(sys.argv) > 3 else v7_path.replace('-v7.xcf', '-v9.xcf')

    print(f"V7 restored: {v7_path}")
    print(f"Clean file:  {clean_path}")
    print(f"Output:      {output_path}")
    print(f"Keep from v7: layers matching {KEEP_FROM_V7}")
    ...
    # Build output: v7 base, append clean tile data for non-Wiring layers
    ...
    for li, v7_layer in enumerate(v7_layers):
        ...
        # Skip layers that should keep v7 data
        if should_keep_from_v7(name):
            print(f"  Layer {li:2d} \"{name}\": KEEP v7 ({n_tiles} tiles)")
            total_kept += n_tiles
            continue
        ...
        if clean_entry is None:
            print(f"  Layer {li:2d} \"{name}\": no clean source, keeping {n_tiles} v7 tiles")
            total_kept += n_tiles
            continue
        ...
        if clean_info['w'] != v7_layer['w'] or clean_info['h'] != v7_layer['h']:
            print(f"  Layer {li:2d} \"{name}\": DIMENSION MISMATCH, keeping v7")
            total_kept += n_tiles
            continue

        n_clean = len(clean_tiles)
        if n_tiles != n_clean:
            print(f"  Layer {li:2d} \"{name}\": tile count mismatch (v7={n_tiles} clean={n_clean}), keeping v7")
            total_kept += n_tiles
            continue
        ...
        total_kept += kept
        print(f"  Layer {li:2d} \"{name}\": {redirected} tiles from clean, {kept} kept from v7")
    ...
    for li, loff in enumerate(out_layer_offsets):
        ...
        print(f"  Layer {li:2d} \"{info['name']}\": {n_ok + n_fail} tiles [{status}]")
        if n_fail > 0:
            all_ok = False
